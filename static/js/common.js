...
Public.createStyleVidoHTML = function (style,tokenCode,isPraise) {
	var html = '<div class="article">'
        ...
        +'<p class="name">'+style.memberName+'<span style="font-size:12px;color:#666">  '+style.orgName+'<span></p>';
		if(style.canReport == 0){
			if(currentUserId != style.memberId){
				if(style.reported != null){
					html += '<p style="float: right;" id="report_'+style.styleId+'">已举报</p>';
				}else{
					html += '<p style="float: right;color: #1c4f64;" id="report_'+style.styleId+'" onclick="report('+style.styleId+')">举报</p>';
				}
			}
		}
		html += '<p class="tit">发表作品《'+style.styleTitle+'》</p>';
		...
		if(Public.notEmptyVal(style.videoLogo)){
			html += '<div style="background:url('+fs+ style.videoLogo +');width:200px;margin-left:60px;background-size:cover" ><img src="'+ctx+'/static/img/play.png" width="33%" height="28%"  class="video" style="border: 1px solid #F8F8F8;margin-left:0" ></div>';
			// onclick="showvideo(\''+style.fileUrl+'\',\''+style.memberName+'发表作品\')"
		}else{
			html += '<img src="'+ctx+'/static/img/play.jpg" width="33%" height="28%" style="border: 1px solid #F8F8F8;" class="video" >';
		}
		html += '<div class="show_time">'
		html += '<span class="time">'+style.showTime+'</span>';
		if(isPraise){
...
Public.createStyleMoreImageHTML = function (style,tokenCode,isPraise) {
    var html = '<div class="article">'
		...
		html += '<p class="tit">发表作品《'+style.styleTitle+'》</p>';
		html += '<div class="con">'+style.styleContent+'</div>';
		if(style.styleContent.length >= 34){
			html += '<p class="all" style="color: #2363BE">全文</p>';
		}
		html += '<div class="pics">';
    $.each(style.imagePaths,function (index,impath) {
		...
		+'<a href="'+fs+impath+'"   data-lightbox="'+style.styleId+'" style="background-image:url('+fs+impath+'/200/200)"></a>'
		// +'<a href="'+fs+impath+'"   data-lightbox="'+style.styleId+'" ><img src="'+fs+impath+'"/></a>'
...
Public.createStyleOutVidoHTML = function (style,tokenCode,isPraise) {
	...
	if(style.styleContent.length >= 34){
	html += '<p class="all" style="color: #2363BE">全文</p>';}
    html += '<img src="'+ctx+'/static/img/play.jpg" width="33%" height="28%" style="border: 1px solid #F8F8F8;" class="video" onclick="showvideo(\''+style.fileUrl+'\',\''+style.memberName+'发表作品《'+style.styleTitle+'》\')">';
    html += '<div class="show_time">';
    html += '<span class="time">'+style.showTime+'</span>';
    if(isPraise){
        if(style.isPraise == 'true'){
