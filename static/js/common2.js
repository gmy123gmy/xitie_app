...
Public.createStyleVidoHTML = function (style,tokenCode,isPraise) {
	var html = '<div class="article">'
		...
		if(Public.notEmptyVal(style.videoLogo)){
			html += '<img src="'+fs+ style.videoLogo +'" width="33%" height="28%" style="border: 1px solid #F8F8F8;" class="video" onclick="showvideo(\''+style.fileUrl+'\',\''+style.memberName+'发表作品\')">';
		}else{
			html += '<img src="'+ctx+'/static/img/play.jpg" width="33%" height="28%" style="border: 1px solid #F8F8F8;" class="video" onclick="showvideo(\''+style.fileUrl+'\',\''+style.memberName+'发表作品\')">';
		}
...
Public.createStyleMoreImageHTML = function (style,tokenCode,isPraise) {
    ...
    $.each(style.imagePaths,function (index,impath) {
		...
		+'<a href="'+fs+impath+'" group="'+style.styleId+'" data-lightbox="'+style.styleId+'" style="background-image:url('+fs+impath+')"></a>'
        +'</div>';
    });
    html = html	+'</div>'
    	+'<div class="show_time">'
