...
(function (window, document, JRoll) {
  ...
  var IMG_STYLE = 'width:20px;height:20px;display:block;opacity:0.4;filter:alpha(opacity=40);'
  ...
  JRoll.prototype.pulldown = function (params) {
    ...
    var options = {
      ...
      iconLoading: "<img style='" + IMG_STYLE + "' src=/static/img/loading.gif >",
      iconFinish: "<img style='" + IMG_STYLE + "' src='" + IMG_FINISH + "'>",
      textPull: '<span style="color:#999;font-size:14px">下拉刷新</span>',
      textRelease: '<span style="color:#999;font-size:14px">释放刷新</span>',
      textLoading: '<span style="color:#999;font-size:14px">正在加载</span>',
      textFinish: '<span style="color:#999;font-size:14px">刷新完成</span>',
      spinning: false, // 应网友要求添加一个选项控制loading时是否旋转icon
    ...
    boxDiv.style.cssText = 'position:absolute;top:-44px;width:100%;height:44px;line-height:44px;font-size:14px;text-align:center;'

    iconSpan = document.createElement('span')
    iconSpan.className = 'jroll-plugin-pulldown-icon'
    iconSpan.style.cssText = 'display:inline-block;width:24px;height:24px;position:absolute;top:10px;left:35%;'
